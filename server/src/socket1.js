...
export function registerSocketHandlers(io, socket) {
  ...
  //        SEND MESSAGE
  // -----------------------------
  socket.on('message:send', async ({ conversationId, body, attachment }) => {
    try {
      ...
      if (
        !convo ||
        !convo.participants.map((id) => String(id)).includes(socket.userId)
      ) {
        console.log('[socket] user not in conversation, abort', {
          conversationId,
      ...
      // במקום לחסום, רק נרשום לוג אזהרה אם המשתמש לא מופיע כמשתתף
      if (!convo.participants.map((id) => String(id)).includes(socket.userId)) {
        console.warn('[socket] WARNING: user not in participants array, but continuing anyway', {
      ...
      const payload = await Message.findById(msg._id).populate(
        'sender',
      ...
      // 1) לכל מי שבחדר של השיחה
      io.to(convoRoom(conversationId)).emit('message:new', payload);

      // 2) לכל המשתתפים דרך userRoom – גם conversation:update וגם message:new
